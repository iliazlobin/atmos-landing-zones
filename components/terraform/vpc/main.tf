locals {
  ...
  gw_subnets_additional_tags = {
    (var.subnet_type_tag_key)         = "gw",
    "kubernetes.io/role/internal-elb" = 1,
  }

  service_subnets_additional_tags = {
    (var.subnet_type_tag_key)         = "service",
    "kubernetes.io/role/internal-elb" = 1,
  }

  inspection_subnets_additional_tags = {
    (var.subnet_type_tag_key)         = "inspection",
    "kubernetes.io/role/internal-elb" = 1,
  }
  ...
  data_subnets_additional_tags = {
    (var.subnet_type_tag_key)         = "data",
    "kubernetes.io/role/internal-elb" = 1,
  }
...
locals {
  sync_states = can(aws_networkfirewall_firewall.this[0].firewall_status[0].sync_states) ? aws_networkfirewall_firewall.this[0].firewall_status[0].sync_states : []
  inspection_firewall_endpoints = [ for s in local.sync_states
    : s.attachment[0].endpoint_id if can(s.attachment[0].endpoint_id)
  ]
}

output "debug_inspection_firewall_endpoints" {
  value = local.inspection_firewall_endpoints
}
...
module "subnets" {
  ...
  gw_subnets_enabled                 = var.gw_subnets_enabled
  gw_subnets_additional_tags         = local.gw_subnets_additional_tags
  service_subnets_enabled            = var.service_subnets_enabled
  service_subnets_additional_tags    = local.service_subnets_additional_tags
  inspection_subnets_enabled         = var.inspection_subnets_enabled
  inspection_subnets_additional_tags = local.inspection_subnets_additional_tags
  inspection_firewall_endpoints      = local.inspection_firewall_endpoints
  ...
  data_subnets_enabled               = var.data_subnets_enabled
  data_subnets_additional_tags       = local.data_subnets_additional_tags
  vpc_id                             = module.vpc.vpc_id
