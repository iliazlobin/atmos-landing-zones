...
# This file generates a permission set for each role specified in var.identity_roles_accessible
...
data "aws_iam_policy_document" "assume_identity_role" {
  ...
  statement {
    ...
    resources = [
      ...
      format("arn:${local.aws_partition}:iam::*:role/*-%s", each.value)
    ]
...
# locals {
#  debug = [
#   for each in var.identity_roles_accessible
#   : format("arn:${local.aws_partition}:iam::%s:role/%s-%s", local.identity_account, module.this.id, each)
#  ]
# }
