...
locals {
  ...
  filtered_accounts = [
    ...
    : acct if can(index(var.exclude_accounts, acct.name)) == false
  ]

  full_account_map = {
    for acct in local.filtered_accounts
    : can(lookup(var.full_account_remap, acct.name)) ? lookup(var.full_account_remap, acct.name) : acct.name => acct.id
  }
