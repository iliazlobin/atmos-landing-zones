...
resource "aws_organizations_account" "organization_accounts" {
  ...
  name                       = can(each.value.aws_name) ? each.value.aws_name : each.value.name
  # email                      = format(each.value.account_email_format, each.value.name)
  email                      = can(each.value.email) ? each.value.email : format(each.value.account_email_format, each.value.name)
  ...
  account_info_map = merge({ for acc in local.all_accounts : acc.name => merge({ for k, v in acc : k => v if k != "name" },
    {
      ...
      id     = can(acc.aws_name) ?  local.account_names_account_ids[acc.aws_name] : local.account_names_account_ids[acc.name]
